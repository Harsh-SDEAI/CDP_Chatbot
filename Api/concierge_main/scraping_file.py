"""
scraping_file.py — Scrape URL → Markdown → FAISS index
"""
...
MD_DIR        = os.path.join(STORAGE_DIR, "md_storage")
FAISS_DIR     = os.path.join(STORAGE_DIR, "faiss_cache")
...
# ── Helpers ───────────────────────────────────────────────────────────────────
def _make_cache_key(session_id: str, url: str, metric: str) -> str:
    return hashlib.sha256(f"{session_id}|{url}|{metric}".encode()).hexdigest()[:24]
...
def _scrape_url(url: str) -> tuple[str, str, str]:
    ...
    return markdown, plain_text, plain_text[:800]


def _save_md(key: str, url: str, markdown: str) -> str:
    path = os.path.join(MD_DIR, f"{key}.md")
...
# ── Public API ────────────────────────────────────────────────────────────────
async def build_index(session_id: str, url: str, similarity_metric: str,
                      cache_dir: str, openai_api_key: str) -> dict:
    os.makedirs(cache_dir, exist_ok=True)
    key        = _make_cache_key(session_id, url, similarity_metric)
    index_path = os.path.join(cache_dir, f"{key}.faiss")
    meta_path  = os.path.join(cache_dir, f"{key}.json")
    client     = AsyncOpenAI(api_key=openai_api_key)

    markdown, plain_text, summary = await asyncio.to_thread(_scrape_url, url)
    chunks = _chunk_text(plain_text)
    if not chunks:
        raise ValueError(f"No content extracted from {url}")

    md_path = await asyncio.to_thread(_save_md, key, url, markdown)
    ...
    index  = faiss.IndexFlatL2(EMBED_DIM) if similarity_metric.upper() == "L2" else faiss.IndexFlatIP(EMBED_DIM)
    index.add(matrix)
    faiss.write_index(index, index_path)

    metadata = {"session_id": session_id, "url": url, "similarity_metric": similarity_metric.upper(),
                 "chunk_count": len(chunks), "summary": summary, "chunks": chunks, "md_path": md_path}
...
def load_index(session_id: str, url: str, similarity_metric: str,
               cache_dir: str) -> Optional[tuple[faiss.Index, dict]]:
    key        = _make_cache_key(session_id, url, similarity_metric)
    index_path = os.path.join(cache_dir, f"{key}.faiss")
    meta_path  = os.path.join(cache_dir, f"{key}.json")
...
# Backwards-compatible aliases (for rag_pipeline.py and scheduler.py)
_embed_texts     = _embed
